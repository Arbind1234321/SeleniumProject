...
public class WritingDataInExcel {
	...
	public static void main(String[] args) throws IOException {
		//1)Create the object of FileOutputStram
		/*FileOutputStream file=new FileOutputStream(System.getProperty("user.dir")+"\\TestData\\excel.xlsx");
		//2)Create workbook object
		XSSFWorkbook workbook=new XSSFWorkbook();
		//3)Create sheet from work book
		XSSFSheet sheet=workbook.createSheet();
		//4)Create row from sheet
		XSSFRow row1=sheet.createRow(0);
		//5) create cell then set cell data to each cell
		row1.createCell(0).setCellValue("Welcome");
		row1.createCell(1).setCellValue("1234");
		row1.createCell(2).setCellValue("4556");
		XSSFRow row2=sheet.createRow(1);
		row1.createCell(0).setCellValue("Arbind");
